...
QtObject {
    ...
    property var _fallbackConfigFile: FileView {
        path: homeDir + "/.config/quickshell/data/config.json"
        watchChanges: true
        onFileChanged: _fallbackConfigFile.reload()
    }
    property string _rawText: _configFile.__text ?? ""
    property string _fallbackRawText: _fallbackConfigFile.__text ?? ""
    ...
    property var _fallbackData: {
        var raw = _fallbackRawText
        if (!raw) return {}
        try { return JSON.parse(raw) }
        catch (e) { return {} }
    }

    property var _envFile: FileView {
        path: BootstrapService.ready ? (configDir + "/.env") : ""
        watchChanges: true
        onFileChanged: _envFile.reload()
    }
    property string _rawEnv: _envFile.__text ?? ""
    property var _envMap: {
        var text = _rawEnv || ""
        if (!text) return {}
        var map = {}
    ...
    function _detectWeAssetsDir() {
        var configuredSteam = _resolve(_data.paths?.steam)
        if (configuredSteam)
            return configuredSteam + "/steamapps/common/wallpaper_engine/assets"

        var localSteam = homeDir + "/.local/share/Steam/steamapps/common/wallpaper_engine/assets"
        return localSteam
    }

    function _detectWeBinary() {
        var configured = _resolve(_data.paths?.steamWeBinary)
        if (configured)
            return configured
        if (_resolve(_data.paths?.weBinary))
            return _resolve(_data.paths?.weBinary)

        // Default Arch/AUR install path; avoids PATH issues on early session startup.
        return "/usr/bin/linux-wallpaperengine"
    }

    readonly property string weAssetsDir: _resolve(_data.paths?.steamWeAssets)
        || _detectWeAssetsDir()
    readonly property string weBinary: Quickshell.env("SKWD_WE_BINARY")
        || (_envMap.SKWD_WE_BINARY ?? "")
        || _detectWeBinary()
    readonly property string steamDir: _resolve(_data.paths?.steam)

    readonly property string mainMonitor: _data.monitor ?? ""
    readonly property string ollamaUrl: Quickshell.env("SKWD_OLLAMA_URL") || (_envMap.SKWD_OLLAMA_URL ?? "") || (_data.ollama?.url ?? "")
    readonly property string ollamaModel: _data.ollama?.model ?? ""

    readonly property string steamApiKey: Quickshell.env("STEAM_API_KEY") || (_envMap.STEAM_API_KEY ?? "") || (_data.steam?.apiKey ?? "")
    ...
    function _normalizeIntegrations(raw) {
        if (!raw) return []
        ...
        var templateMap = {
            ...
            vscode: "vscode-theme.json",
            vesktop: "vesktop.css",
            zen: "zen.css",
            spicetify: "spicetify.ini",
            spicetifyCss: "spicetify.css",
            yazi: "yazi-theme.toml",
            qt6ct: "qt6ct-colors.conf",
            niri: "niri-colors.kdl",
            ghostty: "ghostty.conf"
        }

        var reloadMap = {
            kde: installDir + "/data/scripts/reload-kde.sh " + (_data.matugen?.kdeColorScheme ?? "SkwdMatugen"),
            niri: installDir + "/data/scripts/reload-niri.sh",
            spicetify: installDir + "/data/scripts/reload-spicetify.sh",
            spicetifyCss: installDir + "/data/scripts/reload-spicetify.sh"
        }
        ...
        for (var key in raw) {
            ...
            if (reloadMap[key]) entry.reload = reloadMap[key]
            list.push(entry)
        }
        return list
    }

    readonly property var integrations: _normalizeIntegrations(_data.integrations || _fallbackData.integrations)
    ...
    readonly property string wallpaperFilterBarBgColor: _wallpaperSelector.filterBarBgColor ?? ""
    ...
    readonly property string wallpaperActiveCustomPreset: _wallpaperSelector.activeCustomPreset ?? ""
    ...
    readonly property string wallhavenApiKey: Quickshell.env("WALLHAVEN_API_KEY") || (_envMap.WALLHAVEN_API_KEY ?? "") || (_data.wallhaven?.apiKey ?? "")

    readonly property bool wallpaperAutoChangeEnabled: _wallpaperSelector.autoChangeEnabled === true
    readonly property string wallpaperAutoChangeMode: {
        var mode = _wallpaperSelector.autoChangeMode
        return (mode === "next" || mode === "random") ? mode : "random"
    }
    readonly property int wallpaperAutoChangeIntervalMinutes: {
        var raw = _wallpaperSelector.autoChangeIntervalMinutes
        var parsed = parseInt(raw !== undefined ? raw : 10)
