...
Scope {
  ...
  WindowSwitcherService {
    id: service
    scriptsDir: windowSwitcher.scriptsDir
    compositor: windowSwitcher.compositor
    configPath: windowSwitcher.configDir + "/data/apps.json"
    homeDir: windowSwitcher.homeDir
    cacheDir: windowSwitcher.cacheDir
    onModelBuilt: idx => {
  ...
  Connections {
    ...
    function onWindowSwitcherVisibleChanged() {
      if (windowSwitcher.showing !== root.windowSwitcherVisible)
        windowSwitcher.showing = root.windowSwitcherVisible
    }
  }
  ...
  onShowingChanged: {
    if (root.windowSwitcherVisible !== showing)
      root.windowSwitcherVisible = showing
    if (showing) {
      waitingForCapture = true
      service.open()
      captureFallbackTimer.restart()
    } else {
  ...
  Timer {
    id: captureFallbackTimer
    interval: 450
    onTriggered: {
      if (windowSwitcher.showing && windowSwitcher.waitingForCapture) {
        windowSwitcher.waitingForCapture = false
  ...
  Connections {
    target: service
    function onScreenshotCounterChanged() {
      if (windowSwitcher.showing && windowSwitcher.waitingForCapture) {
        windowSwitcher.waitingForCapture = false
        captureFallbackTimer.stop()
        cardShowTimer.restart()
      }
    }
  }
  ...
  property bool waitingForCapture: false
      ...
      delegate: Item {
        ...
        Item {
          ...
          Image {
            id: windowThumb
            anchors.fill: parent
            source: (windowSwitcher.compositor === "niri" && model.winId)
              ? "file://" + service.thumbDir + "/" + model.winId + ".png?v=" + service.screenshotCounter
              : ""
